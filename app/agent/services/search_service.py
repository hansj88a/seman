"""상품 검색 5단계 파이프라인: 정규화 → 형태소 → 재구성 → 임베딩 → k-NN 검색."""
from app.core.config import get_settings
from app.core.logging import get_agent_logger, get_exception_location
from app.core.opensearch import get_client
from app.agent.services.embedding_client import get_embedding
from app.agent.services.morph_analyzer import extract_nouns
from app.agent.services.query_reconstructor import reconstruct_query
from app.agent.services.text_normalizer import step1_normalize
...
def search_product_codes(query: str, size: int = 50) -> tuple[list[str], str, str]:
    ...
    try:
        ...
        if not vector:
            logger.warning("4단계 임베딩 실패 또는 빈 벡터")
